...
Set_config_user(){
	...
	read -e -p "(默认: djapple):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="djapple"
...
Set_config_port(){
	...
	read -e -p "(默认: 8885):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="8885"
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	read -e -p "(默认: djapple):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="djapple"
